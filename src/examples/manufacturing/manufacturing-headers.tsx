...
export const HEADERS: HeaderObject[] = [
  {
    ...
    cellRenderer: ({ row }) => {
      ...
      return isLineRow ? (
        <span className="font-bold">{row.productLine}</span>
      ) : (
        <span>{row.productLine}</span>
      );
    },
  },
  {
  ...
  },
  {
    ...
    cellRenderer: ({ row }) => {
      const isLineRow = row.station?.toString().includes("Summary") || false;
      return isLineRow ? (
        <span className="font-bold">{(row.outputRate as number).toLocaleString()}</span>
      ) : (
        <span>{(row.outputRate as number).toLocaleString()}</span>
      );
    },
  },
  {
